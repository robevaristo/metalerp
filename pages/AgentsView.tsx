...
// Função auxiliar para padronizar categoria de barras (reutilizada de Purchasing)
...
const AgentPanel: React.FC<{ 
    ...
    color: string,
    projects: Project[]
}> = ({ type, title, icon, color, projects }) => {
    const [messages, setMessages] = useState<Message[]>([]);
    const [input, setInput] = useState('');
    const [loading, setLoading] = useState(false);
    const [insights, setInsights] = useState<string>('Processando dados...');
    const [loadingInsights, setLoadingInsights] = useState(true);
    const scrollRef = useRef<HTMLDivElement>(null);

    // LÓGICA DE CONSOLIDAÇÃO TÉCNICA
    ...
    const consolidatedItems = rawItems.reduce((acc: any[], current) => {
        // Se for barra, agrupa por categoria técnica + bitola + material
        // Se for chapa, por bitola + material
        // Se for comercial, por nome + material
        let key = "";
        ...
        if (existing) {
            ...
            acc.push({ 
                ...
                key, 
                name: displayName,
                totalMm: current.type === MaterialType.BAR ? (current.quantity * ((current.lengthMm || 0) + 4)) : 0
            });
        }
        return acc;
    }, []);
    ...
    useEffect(() => {
        const fetchInsights = async () => {
            if (projects.length === 0) {
                setInsights("Célula vazia. Aguardando importação.");
            ...
            try {
                ...
                setInsights("Erro ao gerar insights técnicos.");
            } finally {
                setLoadingInsights(false);
            }
        };
    ...
    const handleSend = async () => {
        ...
        try {
            ...
            setMessages(prev => [...prev, { role: 'agent', text: "Falha na resposta." }]);
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="flex flex-col h-[750px] bg-white rounded-[2.5rem] border shadow-xl overflow-hidden animate-fade-in group">
            <div className={`${color} p-6 text-white flex items-center justify-between`}>
                ...
                <div className="px-3 py-1 bg-white/10 rounded-full text-[9px] font-black">{consolidatedItems.length} MATERIAIS BRUTOS</div>
            </div>

            <div className="p-5 bg-slate-900 space-y-3">
                <div className="bg-white/5 border border-white/10 p-4 rounded-xl">
                    <p className="text-[8px] font-black text-teal-400 uppercase mb-2 flex items-center gap-2"><Zap size={10} fill="currentColor"/> Insights da Célula</p>
                    {loadingInsights ? (
                        <div className="flex items-center gap-2 py-1">
                            <Loader2 size={12} className="text-white/20 animate-spin" />
                            <p className="text-[9px] font-bold text-white/30 uppercase animate-pulse">Sintetizando...</p>
                        </div>
                    ) : (
            ...
            <div className="flex-1 overflow-y-auto p-5 bg-slate-50/30 space-y-2">
                {consolidatedItems.length === 0 ? (
                    ...
                    consolidatedItems.map((item, idx) => (
                        <div key={idx} className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm flex items-center justify-between group/item">
                            <div className="flex items-center gap-3">
                                <div className="bg-slate-900 text-white w-12 h-12 rounded-lg flex flex-col items-center justify-center shadow-sm">
                                ...
                                <div>
                                    ...
                                    {item.totalMm > 0 && (
                                        <p className="text-[8px] font-bold text-orange-600 uppercase mt-1 tracking-tighter">Carga Linear: {Math.round(item.totalMm)} mm</p>
                                    )}
                                </div>
                            </div>
            ...
            <div className="p-4 bg-white border-t flex gap-2 items-center">
                <input 
                    ...
                    placeholder="Dúvida técnica sobre esta célula..."
                    value={input}
                ...
                <button 
                    onClick={handleSend}
                    disabled={loading}
                    className={`${color} text-white p-3 rounded-xl shadow-lg`}
                >
                    <Send size={16} />
                </button>
            </div>
...
export const AgentsView: React.FC<{ projects: Project[], onImportClick: () => void }> = ({ projects, onImportClick }) => {
    ...
    return (
        <div className="space-y-6 pb-20">
                ...
                <div className="relative z-10 flex flex-col lg:flex-row justify-between items-center gap-6">
                    <div className="flex items-center gap-6">
                        ...
                        <div>
                            ...
                            <p className="text-[9px] font-bold text-teal-400 uppercase tracking-widest mt-2 flex items-center gap-2">
                                <Zap size={10} fill="currentColor"/> Análise de Células de Produção
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="flex gap-4">
                        <button 
                            ...
                            className="bg-white text-slate-900 px-6 py-4 rounded-2xl font-black uppercase text-[10px] shadow-lg flex items-center gap-2 hover:bg-slate-50 transition-all"
                        >
                            <FileSpreadsheet size={16} className="text-indigo-600" /> Carregar Lote
                        </button>
                        <div className="px-5 py-3 bg-white/5 border border-white/10 rounded-2xl flex items-center gap-4">
                             <div>
                                <p className="text-[8px] font-black text-white/40 uppercase mb-1">Total Peças Ativas</p>
            ...
            {projects.length === 0 && (
                <div className="max-w-xl mx-auto text-center p-12 bg-white border-2 border-dashed border-slate-200 rounded-[2.5rem]">
                    <PlusCircle size={32} className="text-slate-200 mx-auto mb-4" />
                    <p className="text-slate-400 font-black uppercase text-[10px] tracking-widest">Nenhum projeto ativo para os agentes analisarem.</p>
                </div>
            )}
        </div>
    );
