...
const App: React.FC = () => {
      ...
      reader.onload = async (evt) => {
          try {
              ...
              if (validRows.length > 0) {
                  ...
                  const headerKeywords = ['QTDE', 'UNID', 'DESCRIÇÃO', 'ITEM', 'POS', 'MATERIAL', 'CLIENTE'];
                  if (headerKeywords.some(kw => firstRowStr.includes(kw))) {
              ...
              if (validRows.length === 0) {
                  setImportError("Planilha vazia ou sem dados legíveis.");
                  setIsProcessing(false);
                  return;
              }

              // Aumentado tamanho do lote para reduzir chamadas totais (economia de cota)
              const chunkSize = 40; 
              const chunks = [];
              ...
              for (let i = 0; i < chunks.length; i++) {
                  setCurrentStep(i + 1);
                  try {
                      // Pausa longa entre lotes para evitar erro 429
                      if (i > 0) await new Promise(r => setTimeout(r, 12000));
                      
                      const result = await processProjectBatch(chunks[i]);
                      if (result?.materials && Array.isArray(result.materials)) {
                          allMaterials = [...allMaterials, ...result.materials];
                      }
                  } catch (batchErr: any) {
                      console.error(`Erro no lote ${i+1}:`, batchErr);
                      // Se algum lote falhar após retentativas, reportamos
                      if (i === 0 && allMaterials.length === 0) {
                          setImportError("O servidor de IA está ocupado. Tente novamente em alguns segundos.");
                          break;
                      }
                  }
              }
              ...
              if (allMaterials.length === 0) {
                  setImportError("A IA não conseguiu reconhecer itens válidos. Verifique se o arquivo contém dados de materiais.");
              } else {
                  setImportResult({ materials: allMaterials.map(m => ({ ...m, tempId: crypto.randomUUID() })) });
              }
              setIsProcessing(false);
          } catch (err: any) {
              setImportError("Erro crítico ao processar Excel: " + err.message);
              setIsProcessing(false);
          }
      };
  ...
  const confirmImport = () => {
      ...
      if (targetProjectId !== "NEW") {
          ...
          setProjects(prev => [{
              id: crypto.randomUUID(), opNumber: op, client: "IMPORTADO", description: "IMPORTAÇÃO IA v.26.13",
              ...
              <div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-[90vw] max-h-[92vh] flex flex-col overflow-hidden">
                  <div className="p-6 border-b bg-white flex justify-between items-center">
                      <div className="flex items-center gap-4">
                          ...
                          <div>
                              <h3 className="text-xl font-black uppercase tracking-tighter text-slate-900">Zindra Importador v.26.13</h3>
                              <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Gestão de Cota e Resiliência Ativada</p>
                          </div>
                      </div>
                  ...
                  <div className="flex-1 overflow-y-auto p-10 bg-slate-50/20">
                      {importError && (
                          <div className="max-w-xl mx-auto bg-red-50 border-2 border-red-200 p-10 rounded-[2rem] text-center space-y-4">
                              ...
                              <h4 className="text-xl font-black text-red-900 uppercase tracking-tighter">Falha no Processamento</h4>
                              <p className="text-xs font-bold text-red-600 uppercase tracking-widest leading-relaxed">{importError}</p>
                              <div className="bg-white/50 p-4 rounded-xl text-[9px] font-bold text-slate-400 uppercase">DICA: Se o erro persistir, verifique sua conexão ou se a planilha está muito complexa.</div>
                              <button onClick={() => setImportError(null)} className="bg-red-600 text-white px-8 py-3 rounded-xl font-black uppercase text-[10px]">Tentar Novamente</button>
                          </div>
                      )}
                      ...
                      {!importResult && !isProcessing && !importError && (
                          <div className="max-w-2xl mx-auto space-y-10">
                              <div onClick={() => fileInputRef.current?.click()} className="border-4 border-dashed border-slate-200 rounded-[3rem] p-24 cursor-pointer hover:border-teal-500 hover:bg-white transition-all bg-slate-50 text-center group">
                                  ...
                                  <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-2">v.26.13 - Modo de Resiliência Ativado</p>
                              </div>
                      ...
                      {isProcessing && (
                          <div className="flex flex-col items-center justify-center py-32 text-center">
                              ...
                              <div className="space-y-4">
                                  <p className="font-black text-slate-900 uppercase text-2xl tracking-tighter">Processando Lote {currentStep} de {totalSteps}</p>
                                  <div className="flex items-center justify-center gap-2 text-indigo-600 font-black uppercase text-[10px] tracking-widest bg-indigo-50 px-4 py-2 rounded-full">
                                      <RefreshCw size={14} className="animate-spin" /> Aguardando resposta segura da IA...
                                  </div>
                                  <p className="text-[8px] font-bold text-slate-400 uppercase">Respeitando limites de cota gratuita (12s entre lotes)</p>
                              </div>
                          </div>
                      )}
                                          ...
                                          {importResult.materials.map((m: any) => (
                                              <tr key={m.tempId} className="hover:bg-slate-50 transition-colors">
                                                  ...
                                                  <td className="p-5">
                                                      ...
                                                      <span className="text-[8px] font-bold text-slate-400 uppercase">REF: {m.gauge} | {m.materialGrade}</span>
                                                  </td>
